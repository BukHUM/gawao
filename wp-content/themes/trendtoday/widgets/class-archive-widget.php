 ...
 */
class TrendToday_Archive_Widget extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'trendtoday_archive',
            __( 'Trend Today: Archive', 'trendtoday' ),
            array( 'description' => __( 'Display monthly archive links', 'trendtoday' ) )
        );
    }

    public function widget( $args, $instance ) {
        $title   = ! empty( $instance['title'] ) ? $instance['title'] : __( 'อาร์คิฟ', 'trendtoday' );
        ...
        if ( $dropdown ) {
            ...
            <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent" name="archive-dropdown" onchange="document.location.href=this.options[this.selectedIndex].value;">
                <option value=""><?php _e( 'เลือกเดือน', 'trendtoday' ); ?></option>
    ...
    public function form( $instance ) {
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>"><?php _e( 'Show post count', 'trendtoday' ); ?></label>
        </p>
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'dropdown' ) ); ?>"><?php _e( 'Display as dropdown', 'trendtoday' ); ?></label>
        </p>
        <?php
    }
