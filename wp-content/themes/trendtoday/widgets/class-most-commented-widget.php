 ...
 */
class TrendToday_Most_Commented_Widget extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'trendtoday_most_commented',
            __( 'Trend Today: Most Commented', 'trendtoday' ),
            array( 'description' => __( 'Display posts with most comments', 'trendtoday' ) )
        );
    }

    public function widget( $args, $instance ) {
        $title  = ! empty( $instance['title'] ) ? $instance['title'] : __( 'บทความที่มีความเห็นมาก', 'trendtoday' );
            ...
            <a href="<?php echo esc_url( trendtoday_fix_url( get_permalink() ) ); ?>" 
               ...
               role="listitem">
                <div class="flex-grow min-w-0">
                    ...
                    <?php if ( $show_count ) : ?>
                        <span class="text-xs text-gray-500"><?php echo (int) $comments; ?> <?php _e( 'ความเห็น', 'trendtoday' ); ?></span>
                    <?php endif; ?>
                </div>
            </a>
            <?php
    ...
    public function form( $instance ) {
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_count' ) ); ?>"><?php _e( 'Show comment count', 'trendtoday' ); ?></label>
        </p>
        <?php
    }
