 ...
 */
class TrendToday_Related_Posts_Widget extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'trendtoday_related_posts',
            __( 'Trend Today: Related Posts', 'trendtoday' ),
            array( 'description' => __( 'Display related posts by category/tag (best on single post)', 'trendtoday' ) )
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'บทความที่เกี่ยวข้อง', 'trendtoday' );
               ...
               role="listitem">
                <?php if ( has_post_thumbnail() ) : ?>
                    <div class="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-gray-100">
                        <?php the_post_thumbnail( 'trendtoday-thumbnail', array( 'class' => 'w-full h-full object-cover', 'alt' => $post_title ) ); ?>
                    </div>
                <?php else : ?>
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of posts:', 'trendtoday' ); ?></label>
        ...
        <p class="description"><?php _e( 'Shows posts from the same category/tag. Use on single post sidebar.', 'trendtoday' ); ?></p>
        <?php
    }
